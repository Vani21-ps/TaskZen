...
export function CalendarView() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const formattedEvents: TaskEvent[] = response.data.map((task) => ({
        _id: task._id,
        title: task.title,
        start: task.dueDate ? new Date(task.dueDate) : new Date(), // Use dueDate for start
        end: task.dueDate ? new Date(task.dueDate) : new Date(), // For simplicity, end is same as start
  ...
  const handleTaskFormClose = () => {
    ...
    setIsFormOpen(false)
    fetchTasks() // Refresh events after create/edit
  }
    ...
    <div className="h-full">
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <TaskForm task={selectedTask} onTaskCreated={handleTaskFormClose} />
        </DialogContent>
      </Dialog>
    </div>
  )
